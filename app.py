...
# from curses.ascii import FF  # ❌ REMOVE THIS LINE
FF = '\f'  # ✅ Add this instead (only if you use FF later)
...
scheduler.add_job(
    func=auto_offline_check,  # type: ignore
    trigger="interval", 
...
def generate_user_id(email):
    ...
    Example: akesharwani780@gmail.com -> akesharwani_4673
    """
    try:
        if not email:
        ...
        # Clean username - remove dots, numbers, special chars
        ...
        # Check if this userId already exists in database
        if db:
            existing_check = db.collection("users").where(
                filter=FieldFilter("userId", "==", user_id)
            ).limit(1).get(timeout=5)
            
            # If exists, regenerate with different number
...
def get_user_by_uid(uid):
    ...
    try:
        ...
        # ✅ FIX 1: Reduced timeout from 10s to 5s
        user_doc = db.collection("users").document(uid).get(timeout=5)
        ...
        # ✅ FIX 2: Fix invalid or old format userId
        if not user_id or user_id.startswith("USER_"):
            user_id = generate_user_id(email)
            referral_code = user_data.get("referralCode") or generate_referral_code()
            
            # ✅ FIX 3: Async update (don't block response)
            try:
        ...
        # ✅ FIX 4: Return only necessary fields (faster JSON serialization)
        return jsonify({
            "uid": uid,
            "email": user_data.get("email"),
            "displayName": user_data.get("displayName"),
...
def set_user_online(uid):
    ...
    try:
        ...
        # ✅ FIX 1: Prepare base updates
        updates = {
            "isOnline": is_online,
            "lastActiveAt": current_time.isoformat()
        }

        # ✅ FIX 2: Only fetch user data if going online (optimization)
        if is_online:
            user_doc = user_ref.get(timeout=3)  # Add timeout
            if user_doc.exists:
        ...
        # ✅ FIX 3: Add timeout to set operation
        ...
        # ✅ FIX 4: Don't fail on timeout, just log
        if "deadline exceeded" in str(e).lower() or "timeout" in str(e).lower():
...
def auto_offline_check():
    ...
    try:
        ...
        # ✅ FIX 5: Query only online users (MASSIVE optimization)
        users_ref = db.collection("users") \
            .where(filter=FieldFilter("isOnline", "==", True)) \
            .stream(timeout=10)
        
        offline_count = 0
        batch = db.batch()  # ✅ FIX 6: Use batch writes (faster)
        batch_count = 0
        ...
        for doc in users_ref:
            ...
            if last_active:
                try:
                    ...
                    # ✅ FIX 7: Check if user is actually inactive
                    if (now_time - last_active_dt) > inactive_threshold:
                        batch.update(doc.reference, {"isOnline": False})
                        batch_count += 1
                        offline_count += 1
                        
                        # ✅ FIX 8: Commit batch every 500 updates (Firestore limit)
                        if batch_count >= 500:
                            batch.commit()
                            batch = db.batch()
                            batch_count = 0
        ...
        # ✅ FIX 9: Commit remaining updates
        if batch_count > 0:
            batch.commit()
...
# ==================== Ping Endpoint (NEW - for keeping users online) ====================
...
def save_subscription():
    ...
    try:
        ...
        # ✅ 1. Save subscription in subscriptions collection
        subscription_data = {
            "userId": user_id,
            "plan": plan,
            "status": "active",
        ...
        # ✅ 2. UPDATE USER DOCUMENT - This is the fix!
        try:
            # Find user by userId (custom ID like "anik_3636")
            users_query = db.collection("users").where(
                filter=FieldFilter("userId", "==", user_id)
            ).limit(1).stream()
            ...
            if user_doc:
                ...
                # User might not exist yet - this is OK, subscription is still saved
                
        except Exception as user_update_error:
            print(f"⚠️ User update error (subscription still saved): {user_update_error}")
            # Don't fail the request - subscription is saved, user update is secondary
...
def get_user_subscriptions(user_id):
    ...
    try:
        ...
        # ✅ FIX 1: Add timeout + limit results
        subs_ref = db.collection("subscriptions").where(
            filter=FieldFilter("userId", "==", user_id)
        ).limit(10).stream(timeout=5)  # Limit to 10 + 5s timeout
...
def check_subscription(user_id):
    ...
    try:
            ...
            if expires_at:
                try:
                    ...
                    if expiry_date > now_time:
                        ...
                        break  # ✅ Found active subscription, break early
                except Exception as e:
        ...
        # ✅ FIX 4: Return safe default on error
        if "deadline exceeded" in str(e).lower() or "timeout" in str(e).lower():
            return jsonify({'isPremium': False}), 200
...
if __name__ == "__main__":
    ...
    if os.environ.get("FLASK_ENV") != "production":
        ...
        print("✅ Use gunicorn for production: gunicorn app:app")